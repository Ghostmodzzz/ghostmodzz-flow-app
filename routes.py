...
def dashboard():
    ...
    for pc in pcs:
        ...
        for b in bills:
            ...
            rem -= take
        allocations.append({'date': pc.date, 'amount': float(pc.amount), 'allocations': allocs})

    return render_template('dashboard.html', bills=bills, paychecks=allocations,
                           bill_form=bill_form, pc_form=pc_form)
